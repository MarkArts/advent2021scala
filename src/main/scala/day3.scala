...
def filterBitsByIndex(compare: (Int, Int) => Int, default: Int)(i: Int, diagnostic: Diagnostic): Option[Int] =
  ...
  Some({
    ...
    val a = uniques(0)
    val b = uniques.get(1) match { 
      case Some(v) => v 
      case None => a
    }
    
    if      (compare(a, b) > 0) 0
    else if (compare(a, b) < 0) 1
    else    default
  })
...
def getLeastCommonBit(i: Int, diagnostic: Diagnostic): Option[Int] =
  filterBitsByIndex((a,b) => b - a, 0)(i, diagnostic)
